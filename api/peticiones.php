...
if ($method === 'GET') {
    ...
    if (isset($_GET['id'])) {
        ...
        try {
            ...
            // ✅ OPTIMIZADO: Parsear datos de GROUP_CONCAT una sola vez
            $peticiones_arr = array("records" => []);

            foreach ($peticiones as $row) {
                // Parsear departamentos
                $departamentos = [];
                if (!empty($row['departamentos_data'])) {
                    $deptItems = explode('@@', $row['departamentos_data']);
                    foreach ($deptItems as $item) {
                        $parts = explode('||', $item);
                        if (count($parts) >= 5) {
                            $departamentos[] = [
                                'asignacion_id' => $parts[0],
                                'departamento_id' => $parts[1],
                                'estado_asignacion' => $parts[2],
                                'fecha_asignacion' => $parts[3],
                                'nombre_unidad' => $parts[4]
                            ];
                        }
                    }
                }
                ...
                // Parsear sugerencias
                $sugerencias = [];
                if (!empty($row['sugerencias_data'])) {
                    $sugItems = explode('@@', $row['sugerencias_data']);
                    foreach ($sugItems as $item) {
                        $parts = explode('||', $item);
                        if (count($parts) >= 4) {
                            $sugerencias[] = [
                                'id' => $parts[0],
                                'departamento_nombre' => $parts[1],
                                'estado' => $parts[2],
                                'fecha' => $parts[3]
                            ];
                        }
                ...
                // Remover campos temporales de GROUP_CONCAT
                unset($row['departamentos_data']);
                unset($row['sugerencias_data']);

                $peticion_item = $row;
                $peticion_item['departamentos'] = $departamentos;
                $peticion_item['sugerencias_ia'] = $sugerencias;
